...
function buildPeriodOptions(): PeriodOption[] {
  ...
  // Mois précédents (jusqu'à 6 mois en arrière)
  for (let i = 1; i <= 6; i++) {
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button z-button zType="outline" zSize="sm" (click)="loadAll()" [disabled]="loading()">
            <z-icon
              [zType]="loading() ? 'loader-2' : 'rotate-ccw'"
      ...
      @if (loading()) {
        <div class="flex flex-col items-center justify-center py-16 gap-4">
          <z-icon zType="loader-2" class="h-10 w-10 text-primary animate-spin" />
      ...
      <!-- ── Erreur ── -->
        ...
        <z-card class="p-8 text-center border-destructive/50 bg-destructive/5">
          <z-icon zType="circle-alert" class="h-12 w-12 text-destructive mx-auto mb-3" />
        ...
        <div class="space-y-6">
          ...
          <z-card class="p-5 shadow-sm rounded-xl overflow-hidden">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-xs text-muted-foreground bg-muted/40 px-2 py-1 rounded-full">sur 100 pts</span>
            </div>
            @if (scoreChartOptions(); as opt) {
              <apx-chart
                ...
                [markers]="$any(opt).markers"
              />
            }
          </z-card>

          <!-- Incidents + Médicaments côte à côte -->
          <div class="grid md:grid-cols-2 gap-6">

            <z-card class="p-5 shadow-sm rounded-xl overflow-hidden">
              <h3 class="font-semibold text-base mb-4">Incidents par type</h3>
            ...
            <z-card class="p-5 shadow-sm rounded-xl overflow-hidden">
              <h3 class="font-semibold text-base mb-4">Observance médicaments</h3>
          ...
          <!-- Hydratation -->
          <z-card class="p-5 shadow-sm rounded-xl overflow-hidden">
            <h3 class="font-semibold text-base mb-4">Hydratation quotidienne (ml)</h3>
          ...
          <!-- Activité -->
          <z-card class="p-5 shadow-sm rounded-xl overflow-hidden">
            <h3 class="font-semibold text-base mb-4">Activité physique cumulée (min/jour)</h3>
...
export class StatisticsDashboardComponent implements OnInit {
  ...
  scoreChartOptions = computed((): Partial<ApexOptions> | null => {
    ...
    // Couleur point par point selon le niveau
    ...
    return {
      ...
      chart:  { type: 'line', height: 300, animations: { enabled: false }, toolbar: { show: false }, fontFamily: 'inherit' },
  ...
  incidentChartOptions = computed((): Partial<ApexOptions> | null => {
    ...
    return {
      ...
      chart:  { type: 'bar', height: 260, animations: { enabled: false }, toolbar: { show: false }, fontFamily: 'inherit' },
  ...
  medicationChartOptions = computed((): Partial<ApexOptions> | null => {
    ...
    return {
      ...
      chart:  { type: 'donut', height: 260, animations: { enabled: false }, fontFamily: 'inherit' },
  ...
  hydrationChartOptions = computed((): Partial<ApexOptions> | null => {
    ...
    return {
      ...
      chart:  { type: 'area', height: 280, animations: { enabled: false }, toolbar: { show: false }, fontFamily: 'inherit' },
  ...
  activityChartOptions = computed((): Partial<ApexOptions> | null => {
    ...
    return {
      ...
      chart:  { type: 'bar', height: 280, animations: { enabled: false }, toolbar: { show: false }, fontFamily: 'inherit' },
      plotOptions: { bar: { horizontal: false, columnWidth: '55%', borderRadius: 5 } },
      xaxis:  { categories: d.dates, labels: { rotate: -45, style: { fontSize: '11px' } }, tickAmount: Math.min(d.dates.length, 15) },
      colors: [C.success],
